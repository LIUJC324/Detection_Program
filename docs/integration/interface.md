...
1. `rgbPullUrl` 和 `irPullUrl` 一起构成双路输入，二者必须同时传。
2. 历史兼容字段 `sourceUrl` 仍可接受，但新联调口径应优先使用双路字段。
3. `sampleFps` 仍然保留，建议显式传；不传则使用模型端部署默认值。
4. 为兼容过渡，模型端也接受旧写法 `rgbPushUrl / irPushUrl`。
5. 除 `sourceUrl` 的替换外，其他辅助字段口径保持不变。
