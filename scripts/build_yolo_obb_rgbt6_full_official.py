...
DEFAULT_TRAIN_THERMAL_DIR = PROJECT_ROOT / "datasets" / "raw" / "dronevehicle" / "train_unpack" / "train" / "trainimgr"
DEFAULT_VAL_THERMAL_DIR = PROJECT_ROOT / "datasets" / "raw" / "dronevehicle" / "val_unpack" / "val" / "valimgr"
DEFAULT_TARGET_ROOT = PROJECT_ROOT / "datasets" / "yolo_obb_official_rgbt6_full_v1"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build full official RGB-T 6ch OBB dataset with jpg symlinks and npy sidecars.")
    parser.add_argument("--source-root", type=Path, default=DEFAULT_SOURCE_ROOT)
    parser.add_argument("--train-thermal-dir", type=Path, default=DEFAULT_TRAIN_THERMAL_DIR)
    parser.add_argument("--val-thermal-dir", type=Path, default=DEFAULT_VAL_THERMAL_DIR)
...
def main() -> None:
    ...
    train_thermal_dir = args.train_thermal_dir.resolve()
    val_thermal_dir = args.val_thermal_dir.resolve()
    target_root = args.target_root.resolve()

    written = {"train": 0, "val": 0}
    missing = {"train": 0, "val": 0}

    for split, thermal_dir in (("train", train_thermal_dir), ("val", val_thermal_dir)):
        ...
        for rgb_path in sorted(src_img_dir.glob("*_rgb.jpg")):
            ...
            thermal_path = thermal_dir / f"{stem}.jpg"
    ...
    summary = {
        ...
        "train_thermal_dir": str(train_thermal_dir),
        "val_thermal_dir": str(val_thermal_dir),
